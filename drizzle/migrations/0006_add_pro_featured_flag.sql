-- Only an admin can put a firm on the homepage. Nothing is featured by
-- default, so the homepage shows no "featured tradesmen" until real,
-- hand-picked firms exist.
ALTER TABLE public.pros
  ADD COLUMN IF NOT EXISTS featured boolean NOT NULL DEFAULT false;

CREATE OR REPLACE FUNCTION public.enforce_pro_publish_approval()
RETURNS trigger
LANGUAGE plpgsql
SET search_path TO 'public'
AS $function$
BEGIN
  IF public.has_role(auth.uid(), 'admin'::app_role) THEN
    RETURN NEW;
  END IF;
  IF TG_OP = 'INSERT' THEN
    NEW.published := false;
    NEW.featured := false;
  ELSE
    IF NEW.published IS DISTINCT FROM OLD.published THEN
      NEW.published := OLD.published;
    END IF;
    IF NEW.featured IS DISTINCT FROM OLD.featured THEN
      NEW.featured := OLD.featured;
    END IF;
  END IF;
  RETURN NEW;
END;
$function$;